...
static void statitem_load(statitem_t *arr, size_t n, const char *path) {
  ...
  if( (file = fopen(path, "r")) == 0 ) {
    dsme_log(LOG_ERR, "can't load %s: %s\n", path, strerror(errno));
  }
  else {
...
static int statitem_save(const statitem_t *arr, size_t n, const char *path) {
  ...
  if( (file = fopen(path, "w")) == 0 ) {
    dsme_log(LOG_ERR, "can't save %s: %s\n", path, strerror(errno));
    goto cleanup;
  }
  ...
  if( ferror(file) || fflush(file) == EOF ) {
    dsme_log(LOG_ERR, "can't write %s: %s\n", path, strerror(errno));
    goto cleanup;
  }
  
  if( fsync(fileno(file)) == -1 ) {
    dsme_log(LOG_ERR, "can't sync %s: %s\n", path, strerror(errno));
    goto cleanup;
  }
  ...
  if( file != 0 && fclose(file) == EOF )
    dsme_log(LOG_WARNING, "can't close %s: %s\n", path, strerror(errno));
...
DSME_HANDLER(DSM_MSGTYPE_PROCESS_STOP, client, msg)
{
  ...
  while ((ptr = g_slist_find_custom(ptr, command, compare_commands))) {
      ...
      if( seteuid(oldeuid) == -1 ) {
          dsme_log(LOG_ERR, "seteuid(%d): %m", oldeuid);
      }
...
static int reboot_flag(void)
{
	...
	return ret;
#else
	dsme_log(LOG_WARNING, "NO CAL SUPPORT; lifeguard reboots disabled");
	return 0;
#endif
}
