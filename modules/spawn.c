...
#define DSME_EXEC_HELPER_PATH "/usr/sbin/dsme-exec-helper"
                 ...
                 char*       env[])
{
  ...
  if (make_argv_for_exec_helper(cmdline, uid, gid, nice_val, oom_adj, &args)) {
      ...
      if (pid == 0) {
	      ...
	      fprintf(stderr, "SPAWN '%s' execve() failed: %m\n", cmdline);
          } else {
              execvp(args[0], args);
	      fprintf(stderr, "SPAWN '%s' execvp() failed: %m\n", cmdline);
          }
